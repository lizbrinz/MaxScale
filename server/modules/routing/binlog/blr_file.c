...
blr_read_binlog(ROUTER_INSTANCE *router, BLFILE *file, unsigned long pos, REP_HEADER *hdr, char *errmsg)
{
    ...
    if ((n = pread(file->fd, hdbuf, BINLOG_EVENT_HDR_LEN, pos)) != BINLOG_EVENT_HDR_LEN)
    {
        ...
        {
            case 0:
                MXS_NOTICE("Reached end of binlog file '%s' at %lu.",
                          file->binlogname, pos);
    ...
    if (hdr->next_pos < pos && hdr->event_type != ROTATE_EVENT)
    {
        ...
        if ((n = pread(file->fd, hdbuf, BINLOG_EVENT_HDR_LEN, pos)) != BINLOG_EVENT_HDR_LEN)
        {
            ...
            {
                case 0:
                    MXS_NOTICE("Reached end of binlog file at %lu.",
                              pos);
...
blr_read_events_all_events(ROUTER_INSTANCE *router, int fix, int debug)
{
    ...
    while (1)
    {
        ...
        if (hdr.event_type == FORMAT_DESCRIPTION_EVENT)
        {
            ...
            if(debug)
                MXS_NOTICE("- Format Description event FDE @ %llu, size %lu, time %lu (%s)",
            ...
            if (debug)
            {
                MXS_NOTICE("       FDE ServerVersion [%50s]", ptr + 2);

                MXS_NOTICE("       FDE Header EventLength %i"
            ...
            if (event_header_ntypes < n_events)
            {
                ...
                if (debug)
                    MXS_NOTICE("       FDE Checksum alg desc %i, alg type %s",
                              check_alg,
                              check_alg == 1 ?
            ...
            if (hdr.event_type == MARIADB10_GTID_EVENT)
            {
                ...
                if (flags == 0)
                {
                    ...
                    else
                    {
                        ...
                        if (debug)
                            MXS_NOTICE("> MariaDB 10 Transaction (GTID %u-%u-%lu)"
                                      " starts @ pos %llu",
        ...
        if (hdr.event_type == QUERY_EVENT)
        {
            ...
            if (strncmp(statement_sql, "BEGIN", 5) == 0)
            {
                ...
                else
                {
                    ...
                    if (debug)
                        MXS_NOTICE("> Transaction starts @ pos %llu", pos);
                }
            }
            ...
            if (strncmp(statement_sql, "COMMIT", 6) == 0)
            {
                if (pending_transaction > 0)
                {
                    ...
                    if (debug)
                        MXS_NOTICE("       Transaction @ pos %llu, closing @ %llu",
                                  last_known_commit, pos);
                }
            }
            free(statement_sql);
        ...
        if (hdr.event_type == XID_EVENT)
        {
            ...
            if (pending_transaction > 0)
            {
                ...
                if (debug)
                    MXS_NOTICE("       Transaction XID @ pos %llu, closing @ %llu",
                              last_known_commit, pos);
            }
        }
        ...
        {
            if (debug)
                MXS_NOTICE("< Transaction @ pos %llu, is now closed @ %llu. %lu events seen",
            ...
            /** Clear the current GTID */
            router->current_gtid[0] = '\0';
        }
...
void handle_row_event(ROUTER_INSTANCE *router, REP_HEADER *hdr, HASHTABLE *maphash, uint64_t pos)
{
    ...
    if (map)
    {
        ...
        sprintf(outfile, "/tmp/%s.%s.avro", map->database, map->table);
