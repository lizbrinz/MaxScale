...
static void rwsplit_process_router_options(ROUTER_INSTANCE *router,
                                           char **options);
                                       ...
                                       qc_query_type_t qtype, HINT *hint)
{
    ...
    if (rses->rses_config.rw_strict_multi_stmt &&
            ...
            if (tbl != NULL && tsize > 0)
            {
                ...
                for (i = 0; i < tsize && !target_tmp_table && tbl[i]; i++)
                {
                    ...
                    if (rses_prop_tmp && rses_prop_tmp->rses_prop_data.temp_tables)
                    {
                        ...
                        if ((target_tmp_table = (bool)hashtable_fetch(
                              ...
                              GWBUF *querybuf)
{
        ...
        BREF_IS_CLOSED(rses->rses_master_ref))
    {
        ...
        MXS_ERROR("Can't route %s:%s:\"%s\" to "
                  "backend server. Session doesn't have a Master node",
    ...
    else if (TARGET_IS_SLAVE(route_target))
    {
        ...
        {
            MXS_INFO("Was supposed to route to slavebut finding suitable one failed.");
        }
    }
    else if (TARGET_IS_MASTER(route_target))
    {
        ...
        succp =
            get_dcb(&curr_master_dcb, rses, BE_MASTER, NULL, MAX_RLAG_UNDEFINED);
...
static void diagnostic(ROUTER *instance, DCB *dcb)
{
    ...
    {
        master_pct = (double)router->stats.n_master /
                     (double)(router->stats.n_master + router->stats.n_slave);
    }
    ...
    dcb_printf(dcb, "\tNumber of queries forwarded to all:   	%d\n",
               router->stats.n_all);
    dcb_printf(dcb, "\tMaster/Slave percentage:		%.2f%%\n",
               master_pct * 100.0);
                                           ...
                                           ROUTER_INSTANCE *router)
{
     ...
     * Successful cases
     */
    if (master_connected && slaves_connected >= min_nslaves &&
    ...
    for (i = 0; options[i]; i++)
    {
        if ((value = strchr(options[i], '=')) == NULL)
        {
            MXS_ERROR("Unsupported router option \"%s\" for "
                      "readwritesplit router.", options[i]);
        }
        else
        {
            *value = 0;
            ...
            if (strcmp(options[i], "slave_selection_criteria") == 0)
            {
                ...
                {
                    MXS_WARNING("Unknown slave selection criteria \"%s\". "
            ...
            {
                router->rwsplit_config.rw_disable_sescmd_hist =
                    config_truth_value(value);
            }
